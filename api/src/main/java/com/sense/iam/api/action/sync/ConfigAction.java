...
import com.sense.iam.api.model.sync.ConfigSync;
...
@ApiSort(value = 9)
public class ConfigAction extends AbstractAction<Config,Long>{
	...
	public ResultCode save(@RequestBody ConfigSync entity) {
	...
	public ResultCode edit(@RequestBody ConfigSync entity) {
		if(entity.getId()==0){
			return new ResultCode(Constants.OPERATION_FAIL);
		}else{
			return super.edit(entity.getConfig());
		}
	...
	protected PageList<Config> findList(@RequestBody ConfigReq entity,@RequestParam(defaultValue="1") Integer page,@RequestParam(defaultValue="20") Integer limit){
		try{
			Config p=entity.getConfig();
