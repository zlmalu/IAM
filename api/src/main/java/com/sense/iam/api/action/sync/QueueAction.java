...
import com.sense.iam.api.model.sync.QueueModel;
...
@ApiSort(value = 11)
public class QueueAction extends AbstractAction<Queue,Long>{
	...
	public ResultCode save(@RequestBody Queue entity) {
	...
	public ResultCode edit(@RequestBody Queue entity) {
	...
	@ResponseBody
	public Queue findById(@PathVariable Long id) {
	...
	protected PageList<Queue> findList(@RequestBody QueueModel entity,@RequestParam(defaultValue="1") Integer page,@RequestParam(defaultValue="20") Integer limit){
		try{
			Queue model=new Queue();
			model.setId(entity.getId());
			model.setAppName(StringEscapeUtils.escapeHtml4(entity.getAppName()));
			model.setModuleName(StringEscapeUtils.escapeHtml4(entity.getModuleName()));
			model.setIsLikeQuery(true);
			return getBaseService().findPage(model,page,limit);
		}catch(Exception e){
			log.error("findList error:",e);
			return new PageList<Queue>();
		}
	}
