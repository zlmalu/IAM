...
public class AbstractAction<T extends BaseModel,PK> extends BaseAction{
	...
	protected ResultCode resetSyncs(Params params){
		try{
			if(params.getIds()!=null){
				if(GenericsUtil.getSuperClassGenricType(getClass(), 1)==Long.class){
					getBaseService().restSync((PK[])ArrayUtils.stringArrayToLongArray(params.getIds().toArray(new String[params.getIds().size()])));
				}else{
					getBaseService().restSync((PK[])params.getIds().toArray(new String[params.getIds().size()]));
				}
			}
	...
	protected ResultCode edit(T entity){
		//判断数据是否存在
		try{
			return getBaseService().edit(entity);
		}catch(Exception e){
	...
	protected ResultCode remove(Params params){
		try{
			if(params.getIds()!=null){
				if(GenericsUtil.getSuperClassGenricType(getClass(), 1)==Long.class){
					return getBaseService().removeByIds((PK[])ArrayUtils.stringArrayToLongArray(params.getIds().toArray(new String[params.getIds().size()])));
				}else{
					return getBaseService().removeByIds((PK[])params.getIds().toArray(new String[params.getIds().size()]));
				}
			}
			return new ResultCode(Constants.OPERATION_SUCCESS);
		}catch(Exception e){
