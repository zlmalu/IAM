...
import com.sense.iam.api.model.sys.AcctReq;
...
@ApiSort(value = 26)
public class AcctAction extends AbstractAction<Acct,Long>{
	...
	public ResultCode save(@RequestBody AcctReq entity) {
		Acct acct = entity.getAcct();
		if (sysAcctService.findAcctCountByLoginName(acct.getLoginName()) != 0) {
			return new ResultCode(Constants.OPERATION_EXIST,"该账号已存在！");
		}
		return super.save(entity.getAcct());
	}
	
	@ApiOperation(value="更新",code=1)
	@RequestMapping(value="edit", method=RequestMethod.POST)
	@ResponseBody
	@ApiOperationSupport(order = 2)
	public ResultCode edit(@RequestBody AcctReq entity) {
		Acct acct=entity.getAcct();
		return super.edit(acct);
	}


	@ApiOperation(value="指定唯一标识查询",code=2)
	@ApiImplicitParams({
		 @ApiImplicitParam(name = "id", value = "唯一标识", required = true, paramType="path", dataType = "Long")
	})
	@RequestMapping(value="findById/{id}", method=RequestMethod.GET)
	@ResponseBody
	public Acct findById(@PathVariable Long id) {
		Acct acct=super.findById(id);
		if(acct!=null){
			acct.setRoles(sysAcctService.findSysRoles(id));
		}
		return acct;
	}
	...
	protected PageList<Acct> findList(@RequestBody AcctReq entity,@RequestParam(defaultValue="1") Integer page,@RequestParam(defaultValue="20") Integer limit){
		try{
			Acct acct=entity.getAcct();
