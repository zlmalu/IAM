 ...
 */
@Name("岗位变动清除帐号")
public class CancelPositionClearPower implements SyncInteface{
	@Param("默认sql样例")
...
"<#list accts as acct>${acct.ID?c},</#list>";
	...
	public ResultCode execute(String content) {
		...
		((AccountService)ContextUtil.getBean("imAccountService")).removeByIds(ids.toArray(new Long[ids.size()]));
