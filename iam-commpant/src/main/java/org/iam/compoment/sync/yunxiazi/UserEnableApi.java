 ...
 */
@Name("用户启用/禁用")
public class UserEnableApi extends SyncApi implements SyncInteface {
	...
	@DefaultContent
	private String defaultContent = "<@JDBC id=\"accts\" sql=\"select a.LOGIN_NAME from im_account a where a.ID =${oid}\"/>{\"loginName\": \"${accts[0].LOGIN_NAME}\",\"status\": 1}";
