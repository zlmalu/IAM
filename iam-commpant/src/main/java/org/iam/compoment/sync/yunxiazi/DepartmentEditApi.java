...
public class DepartmentEditApi extends SyncApi implements SyncInteface {
	...
	public ResultCode execute(String content) {
		System.out.println("sync org update content===="+content);
		try{
			JSONObject contObj=JSONObject.fromObject(content);
			
			JSONObject jsonObj = new JSONObject();
			jsonObj.put("id", contObj.getInt("yxzOrgId"));
			jsonObj.put("name", contObj.getInt("name"));
			...
			String result = HttpUtil.POST(addrPrefix+"/3.0/departmentService/editDepartment", body, headers);
