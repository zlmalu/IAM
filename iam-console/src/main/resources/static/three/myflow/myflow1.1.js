(function(b) {
    ...
    a.rect = function(p, m) {
        ...
        function B() {
            ...
            case "image&text":
                ...
                f.attr({
                    x: F + E.img.width,
                    y: r + (o - a.config.lineHeight) / 2 + a.config.lineHeight+15
                }).show();
                e.attr({
                    x: F + E.img.width / 2,
                    y: r + (o - E.img.height) / 2
        ...
        this.toJson = function() {
            ...
            if(Ext.getCmp('east-panel').items.length > 0){
        		var fields=Ext.getCmp('east-panel').items.get(0).getForm().getFields();
        		var len=fields.length;
        		$.panels[Ext.getCmp('east-panel').items.get(0).id]={};
        		for(var i=0;i < len;i++){
        			$.panels[Ext.getCmp('east-panel').items.get(0).id][fields.items[i].name]=fields.items[i].getValue();
        		}
            }
            if(b.panels[this.getId()]){
				for( var key in $.panels[this.getId()] ){
					r += key+ ":{value:\"" + $.panels[this.getId()][key]+ "\"},";
				}
            }
        ...
        this.restore = function(o) {
            ...
            for (var key in o.props) {
            	b.panels[this.getId()][key]=o.props[key].value;
            }
            E = b.extend(true, E, o);
            f.attr({
                text: r.text.text
            ...
            for (var l in m) {
                if (m[l].editor) {
                	...
                	if(Ext.getCmp('east-panel').items.length > 0){
                    	...
                    	Ext.getCmp('east-panel').remove(Ext.getCmp('east-panel').items.get(0).id);
                    }
                    panelConfig(o.getId(),m[l].editor().init(m, l, "p" + l, o, g));
                }
            }
        };
    ...
    var panelConfig=function(id,panel){
    	Ext.getCmp('east-panel').add(panel);
    	Ext.getCmp('east-panel').expand(true);
    	if($.panels[id]){
    		for( var key in $.panels[id] ){
    			panel.getForm().findField(key).setValue($.panels[id][key]);
    		}
        }
    }
